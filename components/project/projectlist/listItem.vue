...
export default {
  ...
  computed: {
    ...
    checked: {
      ...
      set(val) {
        if (val) {
          this.notify(this.$slack.functions.TASK_NOTIFICATION.COMPLETED)
        }
      ...
      set(val) {
        if (val.length > this.tags.length) {
          ...
          if (latestAssignee != this.getCurrentUser) {
            this.notify(this.$slack.functions.TASK_NOTIFICATION.ASSIGNED, {
